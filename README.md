...
由于作者硬件资源有限，本项目未实施全量测试。很多bug需要复制者在本地使用vibe coding进行修改优化。建议使用26年以后参数在25B及以上的模型，使用离你时间线最近开发的25B及以上模型测试。如果模型不够聪明建议投喂更多的相关自动化文件权重参数，不建议频繁更改模型，建议选中模型后，重点反复优化。
