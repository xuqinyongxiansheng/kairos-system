...
logger = logging.getLogger("CoreDecisionLayer")
...
class CoreDecisionLayer:
    ...
    def __init__(self):
        ...
        logger.info("核心决策层初始化完成")
        
    def make_decision(self, evaluation_report: Dict[str, Any]) -> Dict[str, Any]:
        """制定决策"""
    ...
    def _generate_decision(self, evaluation_report: Dict[str, Any], selected_model: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for item in prioritized_info[:2]:
            ...
            if priority_level == 'critical':
                decisions.append({
                    "action": "紧急执行",
                    "target": item.get('item', {}).get('type', 'unknown'),
                    "confidence": 0.95,
        ...
        for recommendation in recommendations:
            decisions.append({
                "action": recommendation.get('action', 'execute'),
    ...
    def _calculate_confidence(self, decisions: List[Dict[str, Any]]) -> float:
        ...
        for decision in decisions:
            total_confidence += decision.get('confidence', 0.7)
    ...
    async def process_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """处理任务"""
        evaluation_report = {
            "value_scores": {"overall": 0.7},
            "recommendations": [
                {"action": "analyze", "reason": "需要分析任务", "priority": "high"}
            ],
            "prioritized_info": [
                {"priority_level": "high", "item": {"type": "task"}}
            ]
        }
        
        return self.make_decision(evaluation_report)


_core_decision_layer = None


def get_core_decision_layer() -> CoreDecisionLayer:
    """获取核心决策层实例"""
    global _core_decision_layer
    
    if _core_decision_layer is None:
        _core_decision_layer = CoreDecisionLayer()
    
    return _core_decision_layer
